...
public class SelectMilkDialogFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        @SuppressWarnings("unchecked") ArrayList<Milk> milks = (ArrayList<Milk>)arguments.getSerializable(KEY_ITEMS);
        Milk selectedMilk = (Milk)arguments.getSerializable(KEY_SELECTED_ITEM);
        
        //表示データを設定
        for (Milk milk : milks) {
            boolean isSelected = milk.equals(selectedMilk) ? true : false;
            adapter.add(new CustomizeNameModel(milk.getMilk(), isSelected));
        }
        
        //クリックイベントを設定
        CustomActivity activity = (CustomActivity)getActivity();
        listView.setOnItemClickListener(new OnSelectMilkItemClickListener(activity, this, milks));
